...
(function(window){
    ...
    //a.初始化食物（放到map中）（在这里创建DOM对象，根据实例化食物对象里面的数据初始化食物）
    var newDiv = null;
    Food.prototype.init= function(map){//创建好的DOM对象放入到参数（map）中；
        ...
        newDiv.style.background = this.background;//背景颜色不带单位
        //加定位和圆角
    ...
    //把局部变量暴露给全局
    window.Food = Food;//全局范围中，有一个名字叫做Food的构造函数;
